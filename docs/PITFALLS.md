...
| 14 | [华为 App 打开记录闪退](#坑14华为-app-打开记录闪退) | 华为端解析/渲染 bug，与 FIT 文件所有可检测属性无关 | 🔴 不可控 |
...
**现象**：192 条记录全部成功导入华为运动健康后，约 8 条记录在手机 App 里**点击打开就闪退**（如某 12km 温哥华跑步、某 5km 昆明跑步）；另有约 7 条记录能打开但**无地图、速度显示天文数字**（1192、1730、2571 甚至 180000）。

**排查：12 个维度全量对比（闪退组 vs 正常组全部零差异）**

1. 开头无效 GPS 点（`0x7FFFFFFF` → lat/lng=180°）数量——排除：192 个文件 156 个都有，正常文件更多（622 个），无图文件 100%，均不闪退
2. 坐标范围 / 海拔 / 缺坐标点 / 相邻跳变
3. 时间戳重复/回退、距离单调性
4. 心率 / 步频 / 温度
5. session/lap 汇总字段（avg_hr=255、max_hr=0 等无效值两组相同）
6. dev 开发者数据字段（全部无）
7. record 字段表结构（含私有字段 87/88，188/192 相同）
8. 87/88 字段值（f87=0/239、f88=100/300，无差异）
9. gmsg 消息类型分布
...
| 实验 | 做法 | 结果 |
|------|------|------|
| 改运动类型 | 跑步(1-0) → 步行(11-0) 重新导入 | 照样闪退 |
| 改时间戳 | file_id.time_created + 全部时间戳平移到新日期，生成「全新记录」绕过华为去重 | 新记录成功出现，仍闪退 |

**根因**：华为运动健康 **App 端对该记录的解析/渲染 bug**，与 FIT 文件的所有可检测属性无关。改时间后华为云端重新解析新记录仍闪退，证明问题在华为端解析过程，客户端不可控。与坑 6（导入 code=400）同类，属华为端问题。

**附带发现**：约 7 条「无 GPS 轨迹」记录（GPS 坐标 100% 缺失：跑步机/室内/信号丢失）在华为能打开但无地图，且速度显示 1192~2571、180000 等天文数字——这是华为对**无轨迹记录的错误显示**（文件里 speed 字段全部正常），非真实速度。

**解决**：无客户端方案。接受现状（少数记录 App 打开闪退，数据本身完整，Garmin/网页端可正常查看）；无图记录的速度离谱值是华为显示逻辑问题，无法在客户端纠正。

**验证**：多个重新导入实验均复现闪退；无图记录的离谱速度与文件 speed 字段无关。
